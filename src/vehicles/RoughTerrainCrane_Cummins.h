...
#include "sounds/Cummins6BTAStart.h" // Cummins 6BTA Diesel
...
#include "sounds/Cummins6BTAIdle.h" // Cummins 6BTA Diesel
...
#ifdef REV_SOUND
#include "sounds/Cummins6BTARev.h" // Cummins 6BTA Diesel
#endif
...
volatile int dieselKnockVolumePercentage = 120; // Adjust the Diesel knock volume (usually = 200 - 600%) 350
...
#include "sounds/Cummins6BTAKnock.h" // Cummins 6BTA Diesel

// Adjust the additional turbo sound (set "turboVolumePercentage" to "0", if you don't want it) --------
volatile int turboVolumePercentage = 60; // Adjust the turbo volume (usually = 70%) 120
...
volatile int reversingVolumePercentage = 130; // Adjust the reversing sound volume (usually = 70%)
...
volatile int indicatorVolumePercentage = 80; // Adjust the indicator sound volume (usually = 100%)
...
#define CRANE_MODE // The sound controller is running in crane mode!

// Choose the hydraulic pump sound (uncomment the one you want) --------
volatile int hydraulicPumpVolumePercentage = 40; // Adjust the volume (usually = 120%)70
...
//#include "sounds/Caterpillar323Hydraulic3.h" // Caterpillar 323 excavator (slowed down by factor 1.5)
...
const uint8_t escRampTimeSecondGear = 50; // 50 for King Hauler (this value is always in use for automatic transmission, 40)
...
#define NumberOfAutomaticGears 1 // <<------- Select 1, 2, 3, 4 or 6 gears!
...
uint32_t MAX_RPM_PERCENTAGE = 268; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage!
