...
#include "sounds/KirovetsK700start.h" // Kirovets K700
...
#include "sounds/KirovetsK700idle.h" // Kirovets K700
...
volatile int revVolumePercentage = 88; // Adjust the idle volume (usually = 80%, more also working, depending on sound)
...
volatile const uint16_t idleEndPoint = 250; // above this point, we have 100% rev and 0% idle sound volume (usually 500, min. 50 more than revSwitchPoint)
...
#ifdef REV_SOUND
#include "sounds/KirovetsK700rev.h" // Kirovets K700
#endif
...
#include "sounds/Scania143jakebrake5.h" // SCANIA V8
#endif
...
volatile int dieselKnockVolumePercentage = 36; // Adjust the Diesel knock volume (usually = 40 - 120%)
...
#include "sounds/KirovetsK700knock.h" // Kirovets K700

// Adjust the additional turbo sound (set "turboVolumePercentage" to "0", if you don't want it) --------
volatile int turboVolumePercentage = 24; // Adjust the turbo volume (usually = 14%)
...
volatile int sirenVolumePercentage = 96; // Adjust the siren volume (usually = 80%)
...
//#include "sounds/75mm.h" // 75mm salve
