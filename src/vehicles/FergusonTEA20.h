...
// #include "sounds/FergusonTEA20Start.h"    // Ferguson TEA20 tractor
#include "sounds/FergusonTEA20StartH-7dB.h" // Ferguson TEA20 tractor for small speakers
...
// #include "sounds/FergusonTEA20Idle.h"            // Ferguson TEA20 tractor
#include "sounds/FergusonTEA20IdleH-7dB.h" // Ferguson TEA20 tractor for small speakers
...
volatile const uint16_t revSwitchPoint = 150;                // The rev sound is played instead of the idle sound above this point (180))
...
#ifdef REV_SOUND
// #include "sounds/FergusonTEA20Rev.h" // Ferguson TEA20 tractor
#include "sounds/FergusonTEA20RevH-7dB.h" // Ferguson TEA20 tractor for small speakers
#endif
...
volatile int jakeBrakeVolumePercentage = 0;     // Adjust the max. volume (usually = 150%)
...
volatile int dieselKnockAdaptiveVolumePercentage = 80; // Adjust the Diesel knock volume for the non-first knocks per engine cycle (usually = 50%) 18
...
uint32_t MAX_RPM_PERCENTAGE = 300; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage! In theory 256
