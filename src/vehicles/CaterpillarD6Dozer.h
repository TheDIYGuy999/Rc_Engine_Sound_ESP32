...
volatile int revVolumePercentage = 80; // Adjust the idle volume (usually = 100%, more also working, depending on sound) 90
...
volatile int trackRattleVolumePercentage = 150; // Adjust the volume (usually = 150%) 150
...
volatile int trackRattle2VolumePercentage = 300; // Adjust the volume (usually = 150%)
#ifdef TRACK_RATTLE_2
#include "sounds/D6TrackRattle.h" // CAT D6 dozer track rattle
#endif
...
uint32_t MAX_RPM_PERCENTAGE = 160; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage!
