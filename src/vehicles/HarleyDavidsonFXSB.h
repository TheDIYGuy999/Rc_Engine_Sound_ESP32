...
volatile int idleVolumePercentage = 48; // Adjust the idle volume (usually = 80%, more also working, depending on sound, 40 - 48% if additional diesel knock sound is used)
...
volatile int revVolumePercentage = 48; // Adjust the idle volume (usually = 80%, more also working, depending on sound)
...
volatile int dieselKnockVolumePercentage = 30; // Adjust the Diesel knock volume (usually = 40 - 100%)
volatile int dieselKnockIdleVolumePercentage = 0; // Diesel knock volume while idling (usually = 20%)
volatile int dieselKnockInterval = 4; // Idle sample length divided by this number (4 for V2 engine!)
...
volatile int dieselKnockAdaptiveVolumePercentage = 40; // Adjust the Diesel knock volume for the non-first knocks per engine cycle (usually = 50%)
...
volatile int shiftingVolumePercentage = 0; // Adjust the shifting volume (usually = 40%)
...
uint32_t MAX_RPM_PERCENTAGE = 380; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage!

// Engine mass simulation
const int8_t acc = 4; // Acceleration step (2) 1 = slow for locomotive engine, 9 = fast for trophy truck
const int8_t dec = 2; // Deceleration step (1) 1 = slow for locomotive engine, 5 = fast for trophy truck
