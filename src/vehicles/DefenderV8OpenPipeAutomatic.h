...
#include "sounds/DefenderV8OpenPipeStart.h" // Land Rover Defender V8 Start (long)
...
volatile int fullThrottleVolumePercentage = 120; // Volume Percentage while full throttle (for rev sound as well)
//#include "sounds/DefenderV8Idle.h" // Land Rover Defender V8
#include "sounds/DefenderV8OpenPipeIdle.h" // Land Rover Defender V8
...
#ifdef REV_SOUND
//#include "sounds/DefenderV8Rev.h" // Land Rover Defender V8
#include "sounds/DefenderV8OpenPipeRev.h" // Land Rover Defender V8
#endif
...
volatile int dieselKnockVolumePercentage = 180; // Adjust the Diesel knock volume (usually = 40 - 120%) 140
volatile int dieselKnockIdleVolumePercentage = 0; // Diesel knock volume while no throttle is applied (usually = 20%)
volatile int dieselKnockStartPoint = 100; // Volume will raise above this throttle input( usually 0, for "open pipe" exhaust about 250)
...
#include "sounds/DefenderV8OpenPipeKnock.h" // Land Rover Defender V8 knock
...
#include "sounds/BritishNationalAnthemSiren.h" // The British national anthem ;-)
...
const uint8_t escBrakeSteps = 45; // determines, how fast the ESC is able to brake down (20 - 30, 30 for King Hauler)
...
uint32_t MAX_RPM_PERCENTAGE = 340; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage!
