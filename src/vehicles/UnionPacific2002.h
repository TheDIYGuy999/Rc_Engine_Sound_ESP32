...
#ifdef REV_SOUND
#include "sounds/UnionPacific2002rev.h" // Union Pacific 2002 SD70M Locomotive with 16 cylinder Diesel
#endif
...
volatile int dieselKnockVolumePercentage = 20; // Adjust the Diesel knock volume (usually = 40 - 120%)
volatile int dieselKnockIdleVolumePercentage = 0; // Diesel knock volume while idling (usually = 20%)
volatile int dieselKnockInterval = 16; // Idle sample length divided by this number (number of peaks per engine cycle in idle.h)
...
#include "sounds/UnionPacific2002knock.h" // Union Pacific 2002 SD70M Locomotive with 16 cylinder Diesel

// Adjust the additional turbo sound (set "turboVolumePercentage" to "0", if you don't want it) --------
volatile int turboVolumePercentage = 3; // Adjust the turbo volume (usually = 14%)
volatile int turboIdleVolumePercentage = 100; // the turbo volume will be engine rpm dependent (usually = 10%)
...
const int8_t acc = 1; // Acceleration step (2) 1 = slow for locomotive engine, 9 = fast for trophy truck. Always 2 for automatic transmission!
