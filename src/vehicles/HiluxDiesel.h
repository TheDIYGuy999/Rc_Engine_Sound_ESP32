...
volatile int startVolumePercentage = 112; // Adjust the start volume (usually = 80%)
...
#include "sounds/Landcruiser12HTstart.h" // Toyota Fj40 Landcruiser with 12H Turbo Diesel
...
#include "sounds/Landcruiser12HTidle.h" // Toyota Fj40 Landcruiser with 12H Turbo Diesel
...
#ifdef REV_SOUND
#include "sounds/Landcruiser12HTrev.h" // Toyota Fj40 Landcruiser with 12H Turbo Diesel
#endif
...
volatile int dieselKnockVolumePercentage = 64; // Adjust the Diesel knock volume (usually = 40 - 120%)
volatile int dieselKnockIdleVolumePercentage = 0; // Diesel knock volume while idling (usually = 20%)
volatile int dieselKnockInterval = 6; // Idle sample length divided by this number (usually number of cylinders)
volatile int dieselKnockStartPoint = 150; // Volume will raise above this point ( usually 0, for "open pipe" exhaust about 250)
...
#include "sounds/Landcruiser12HTknock.h" // Toyota Fj40 Landcruiser with 12H Turbo Diesel

// Adjust the additional turbo sound (set "turboVolumePercentage" to "0", if you don't want it) --------
volatile int turboVolumePercentage = 8; // Adjust the turbo volume (usually = 14%)
...
volatile int wastegateVolumePercentage = 20; // Adjust the wastegate volume (usually = 14%, up to 50%)
...
volatile int fanVolumePercentage = 4; // Adjust the fan volume (50% for Tatra 813, else 0%)
...
uint32_t MAX_RPM_PERCENTAGE = 240; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage!
