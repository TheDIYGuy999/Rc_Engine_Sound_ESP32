...
void Dashboard::drawVolt(float volt, float threshold)
{
    ...
    if (abs(volt - lastVal) < 0.1) return; // Prevent display from being triggered too frequently
    if (volt <= threshold) _tft.setTextColor(TFT_RED, TFT_BLACK);
    else if (volt <= threshold + 0.5) _tft.setTextColor(TFT_ORANGE, TFT_BLACK);
    else _tft.setTextColor(TFT_WHITE, TFT_BLACK);
