...
volatile int revVolumePercentage = 60; // Adjust the idle volume (usually = 100%, more also working, depending on sound)
...
volatile int dieselKnockVolumePercentage = 150; // Adjust the Diesel knock volume (usually = 200 - 500%)
...
volatile int shiftingVolumePercentage = 0; // Adjust the shifting volume (usually = 200%)
...
const uint16_t pulseSpan = 490; // pulseZero +/- this value (150 for JMT 10A ESC, otherwise around 450)
...
const uint32_t MAX_RPM_PERCENTAGE = 380; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage!
